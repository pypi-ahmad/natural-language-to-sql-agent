...
UI cost estimates use only the selected model identifier and aggregate token
counts already returned by the provider. They do not inspect or expose API
keys, prompt contents, result rows, or provider billing records, and they are
not treated as authoritative invoices.
